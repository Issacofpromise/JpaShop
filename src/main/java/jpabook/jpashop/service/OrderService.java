...
@Transactional public Long order(Long memberId, Long itemId, int count){
Member member = mr.findById(memberId).get();//.findOne(memberId);
Item item = itemRepository.findOne(itemId);Delivery delivery=new Delivery();
delivery.setAddress(member.getAddress());delivery.setStatus(DeliveryStatus.READY);
