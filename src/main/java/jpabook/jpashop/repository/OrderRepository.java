...
public class OrderRepository { private final EntityManager em;
...
criteria.add(status);        }//회원 이름 검색
if (StringUtils.hasText(orderSearch.getMemberName())) { Predicate name =
cb.like(cb.lower(m.<String>get("name")),"%"+orderSearch.getMemberName().toLowerCase()+"%");
criteria.add(name);}
cq.where(cb.and(criteria.toArray(new Predicate[criteria.size()])));
TypedQuery<Order> query = em.createQuery(cq).setMaxResults(1000);
/*List<Order> resultList = query.getResultList();return resultList;    */
...
if (orderSearch.getOrderStatus() != null) {if (isFirstCondition) {
jpql += " where";  isFirstCondition = false;} else { jpql += " and"; }
jpql += " o.status = :status";
} if (StringUtils.hasText(orderSearch.getMemberName())) {
if (isFirstCondition) { jpql += " where"; isFirstCondition = false;
} else {       jpql += " and";       }
jpql += " LOWER(m.name) like LOWER(:name)";
} TypedQuery<Order> query = em.createQuery(jpql, Order.class)
.setMaxResults(1000); if (orderSearch.getOrderStatus() != null) {
query = query.setParameter("status", orderSearch.getOrderStatus());
} if (StringUtils.hasText(orderSearch.getMemberName())) {
query = query.setParameter("name", "%" + orderSearch.getMemberName() + "%");
}return query.getResultList();}public List<Order>findAll(OrderSearch orderSearch){
...
if(!StringUtils.hasText(nameCond))return null;return member.name.contains(nameCond);}
public List<Order> findAllWithItem() {return em.createQuery(
...
public List<Order> findAll() {return em.createQuery(
